...
cp -r "/phenix/hhj3/dcm07e/dAuEnergyScan/ampt_v2.26t5_dau200" $baseOutDir
cd $baseOutDir"ampt_v2.26t5_dau200/"
rm "ana/*"
pwd
...
echo "2.2"				>> input.ampt		# PARJ(41): parameter a in Lund symmetric splitting function   d+Au: 2.2  p+Pb: 0.5
echo "0.5"				>> input.ampt		# PARJ(42): parameter b in Lund symmetric splitting function   d+Au: 0.5  p+Pb: 0.9
...
echo "0.47140452d0"		>> input.ampt		# alpha in parton cascade (D=0.33d0), see parton screening mass  d+Au: 0.47d0  p+Pb: 0.33d0
...
echo "0"				>> input.ampt		# Flag for random orientation of reaction plane (D=0,no; 1,yes)
...
set finalOutDir = "/phenix/plhf/dcm07e/dAuEnergyScan/sims/cumulants"
